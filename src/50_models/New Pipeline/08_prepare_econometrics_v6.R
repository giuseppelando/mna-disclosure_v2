...
if ("time_to_close" %in% names(df_core)) {
  econ_duration <- df_core %>%
    filter(!is.na(completion), completion == 1L,
           !is.na(time_to_close), time_to_close > 0)

  # Create survival event indicator (all = 1, since these are completed)
  econ_duration$event <- 1L

  # Winsorize duration
  if (!"time_to_close_w" %in% names(econ_duration)) {
    econ_duration$time_to_close_w <- winsorize(econ_duration$time_to_close)
  }

  cat(sprintf("  econ_duration: %d obs | mean=%.0f days | median=%.0f days\n",
              nrow(econ_duration),
              mean(econ_duration$time_to_close_w),
              median(econ_duration$time_to_close_w)))

  saveRDS(econ_duration, file.path(CONFIG$path_out, "econ_duration.rds"))
}
